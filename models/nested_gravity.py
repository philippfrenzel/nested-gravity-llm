...
class NestedGravitationalLM(nn.Module):
    ...
    def _update_centers(self, assignments: torch.Tensor, hidden_state: torch.Tensor) -> None:
        with torch.no_grad():
            decay = self.ema_alpha * assignments.mean(dim=0).unsqueeze(-1)
            target = hidden_state.mean(dim=0, keepdim=True).expand_as(self.center_values)
            self.center_values.mul_(1.0 - decay).add_(decay * target)
    ...
    def forward(self, tokens: torch.Tensor, return_metrics: bool = False) -> torch.Tensor | Tuple[torch.Tensor, Dict[str, float]]:
        ...
        for t in range(sequence_length):
            ...
            if self.use_nesting:
                ...
                center_values = self.center_value_proj(self.center_values).unsqueeze(0).expand(batch_size, -1, -1)
                raw_force = (assignments * center_kernel).unsqueeze(-1) * direction * center_values
            ...
            if self.use_nesting:
                self._update_centers(assignments, h_t)
